...
import abraham.web.restcontroller.security.maintaining.beans.SecurityResourceGroupBean;
...
@RestController
@RequestMapping("restapis/security/maintaining/resource")
public class SecurityResourceMaintainingRestController {
    private final Log log = LogFactory.getLog(this.getClass());
    @Autowired
    private SecurityResourceService securityResourceService;

    @RequestMapping(value="group", method=RequestMethod.POST, consumes = MediaTypes.JSON_UTF_8)
    @ResponseStatus(HttpStatus.NO_CONTENT)
    public void addResourceGroup(@RequestBody SecurityResourceGroupBean securityResourceGroupBean) throws AppBizException{
        SecurityResourceGroup securityResourceGroup = new SecurityResourceGroup();
        securityResourceGroup.setName(securityResourceGroupBean.getName());
        securityResourceGroup.setDescription(securityResourceGroupBean.getDescription());
        securityResourceService.addSecurityResourceGroupLast(securityResourceGroup);
    }
